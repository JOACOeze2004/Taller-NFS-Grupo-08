...
private:
    ...
    void send_cars(const Snapshot& snapshot);
    void send_game_data(const Snapshot& snapshot);
    void send_checkpoint_hint(const Snapshot& snapshot);
...
public:
    ...
    void receive_player_config(std::string& name, uint8_t& car_id, std::string& map_name);
    void send_game_init_data(const std::string& map_path, float spawn_x, float spawn_y);
